...
* [Глава 9. Создание конвейеров данных](#глава-9-создание-конвейеров-данных)
  ...
  * [Kafka Connect](#kafka-connect)
    ...
    * [Взглянем на Kafka Connect поближе](#взглянем-на-kafka-connect-поближе)
      ...
      * [Управление смещениями](#управление-смещениями)
...
* [Глава 12. Администрирование Kafka](#глава-12-администрирование-kafka)
  ...
  * [Динамические изменения конфигурации](#динамические-изменения-конфигурации)
    * [Переопределение значений настроек топиков по умолчанию](#переопределение-значений-настроек-топиков-по-умолчанию)
...
* [Глава 13. Мониторинг Kafka](#глава-13-мониторинг-kafka)
  ...
  * [Мониторинг клиентов](#мониторинг-клиентов)
    ...
    * [Показатель потребителей](#показатель-потребителей)
      ...
      * [Показатели координатора потребителя](#показатели-координатора-потребителя)
    * [Квоты](#квоты)
...
Топик с правилами хранения в течение 30 дней может хранить более старые данные, если все данные помещаются в один
сегмент в каждом разделе.
...
Основная идея нового дизайна контроллера заключается в том, что сама Kafka имеет архитектуру, основанную на журналах,
где пользователи представляют состояние в виде потока событий.
...
Запрос должен быть отправлен ведущим репликам указанных в запросе, для этого клиенты предварительно запрашивают
метаданные, чтобы гарантировать правильную маршрутизацию запросов на извлечение. Ведущая реплика проверяет запрос, если
смещение очень старое, то брокер вернет ошибку.
...
Ограничение размера снизу - отправляет только пакет сообщения с заданным размером или больше. Снижает загруженность сети
и процессора. При этом можно задать промежуток времени, больше которого брокеры не ждут и отправляют сообщения.
...
Сообщения, которые не реплицированные на достаточное количество реплик - небезопасны. По этому ответ на запрос может
занимать много времени, если репликация работает с задержкой. Она ограничивается параметром `replica.lag.time.max.ms` -
после исчисление этого времени, реплика будет считаться рассогласованной.

Если потребитель получает сообщения из большого количества партици, то отправка списка всех интересующих его разделов
брокеров занимает много времени и трафика, при этом набор разделов редко меняется и метаданные редко меняются и нужно
возвращать не так много данных. Для этого в Kafka есть кэш сессии выборки. Потребитель может создать кэшированную сессию
из которой можно получить кэшированные данные. Кэш имеет ограниченный размер и Kafka отдает приоритет ведомым репликам и
потребителям с большим набором разделов.
...
* Стоимость. Чем больше у нас копий данных, тем выше затраты на хранение и сеть. Обычно число копий уменьшают. Но это
  снизит также доступность, а долговечность гарантирована устройством хранения
...
Ошибки после отправки сообщения брокеру могут быть: которые можно разрешить повторной отправкой, либо разрешить нельзя.

Если необхдимо не терять ни одного сообщения, то лучше разрешить повторные отправки, оставить количество по умолчанию и
...
Представляет собой сочетание двух понятий: идемпотентный производитель и транзакция.
...
**Атомарная многопартиционная запись** - механизм, который гарантирует целостность операций при одновременной записи
данных в несколько партиций.
...
одинаковым `transactional.id`, но меньшей номером эпохи, то будет игнорироваться.
...
* Коннекторы, отвечают за: определение числа задач для коннектора, разбиение работы по копированию данных между
  задачами, получение от исполнителей настроек для задач и передачу их далее.
* Задачи - отвечают за получение из Kafka и вставку туда данных.
...
* `SASL_SSL` - поддерживается аутентификация(SASL) клиента и сервера, и шифрования, использовать в незащищенных сетях
...
Удобнее всего использовать их во внешней системе посредством подсоединения к процессу Kafka агента-сборщика,
предоставляемого вашей системой мониторинга, который обращается к Kafka с помощью интерфейса JMX.

#### Неприкладыне показатели

Не все показатели можно получить только из JMX:

* Показатели приложения - показатели, которые вы получаете из самой Kafka, из интерфейса JMX
* Журналы - тип данных мониторинга, которые поступают из самой Kafka. Поскольку это не просто число, а некая форма
  текстовых или структурированных данных, они требуют немного большей обработки
* Показатели инфраструктуры - Эти показатели поступают из систем, которые находятся перед Kafka, но все еще в пределах
  пути запроса и под вашим контролем. Примером может служить балансировщик нагрузки
* Синтетические клиенты - Это данные от инструментов, которые являются внешними по отношению к вашему развертыванию
  Kafka, как и клиент, но находятся под вашим непосредственным контролем и обычно не выполняют ту же работу, что и ваши
  клиенты. Внешний монитор, такой как Kafka Monitor, относится к этой категории
...
Показатели для оповещения, будет полезен в течение очень короткого периода времени — как правило,
ненамного дольше, чем время, необходимое для реагирования на проблему.

Показатели для отладки, имеют более длительный временной горизонт, поскольку часто диагностируете проблемы, которые
существуют уже в течение некоторого времени, или углубленно изучаете более сложную проблему.

#### Автоматизация или люди?

Если показатели будут использоваться средствами автоматизации, они должны быть очень конкретными. Но если показатели
будут потребляться людьми, им окажется трудно справиться с представлением большого их количества.
...
* Посредством того, что брокер Kafka оповещает об отсутствии показателей (которые иногда называют устаревшими
  показателями).
...
Service Level Indicator (SLI, индикатор уровня обслуживания) – это количественная оценка работы сервиса, как правило,
связанная с удовлетворенностью
пользователей производительностью приложения или сервиса за заданный период времени (месяц, квартал, год).

Service Level Objectives (SLO, целевой уровень обслуживания) – это желаемое, целевое значение нашего SLI или группы SLI.

Service level agreements (SLA, соглашение об уровне обслуживания) — это контракт между поставщиком услуг и клиентом.

### Какие показатели являются хорошими индикаторами уровня обслуживания

Целевые уровни обслуживания должны описывать, доволен или нет типичный пользователь вашего сервиса, и вы не можете
измерить это субъективно.
...
### Использование целевого уровня обслуживания для оповещения

Целевые уровни обслуживания должны стать базой для ваших основных оповещений. Они описывают проблемы с точки зрения
ваших клиентов и именно они должны волновать вас в первую очередь. Лучший способ использовать целевые уровни
обслуживания для оповещения — это наблюдать за тем, с какой скоростью они выгорают за определенный период времени. Если
ставить их только в качестве оповещения, то может быть довольно поздно. Можно использовать "скорость выгорания целевого
уровня обслуживания" в качестве оповещения.
...
* Проблемы с одним брокером - проявляются как отклонения в показателях кластера и часто связаны с медленными или
  неработающими устройствами хранения данных или ограничениями вычислений со стороны других приложений в системе
* Перегруженные кластеры - если кластер сбалансирован, но многие брокеры показывают повышенную задержку запросов или
  низкий коэффициент простоя пула обработчиков запросов, вы достигли предела возможностей ваших брокеров по обслуживанию
  трафика для этого кластера
* Проблемы с контроллером - сложно диагностировать, и они часто попадают в категорию ошибок в самой Kafka. Эти проблемы
  проявляются в виде рассинхронизации метаданных брокера, неработающих реплик, когда брокеры, казалось бы, в порядке, и
  неправильного выполнения действий по управлению топиками, таких как создание

### Искусство недореплицированных разделов

Этот показатель, для каждого брокера кластера, представляет собой число разделов, для которых этот брокер является
ведущей репликой (ведомые реплики отстают):

* Если многие брокеры кластера сообщают о постоянном (не меняющемся) числе недореплицированных разделов, то обычно это
  значит, что один из брокеров кластера отключен.
* Если число недореплицированных разделов меняется или оно постоянно, но отключенных брокеров нет, то дело обычно в
  проблеме с производительностью кластера.
* Если недореплицированные разделы находятся в одном брокере, то обычно причина проблемы именно в нем и ошибка указывает
  на то, что у других брокеров возникают затруднения при репликации сообщений из него.
* Если недореплицированные разделы есть на нескольких брокерах, дело может быть в проблеме с кластером или с отдельным
  брокером, поскольку у одного из брокеров могут возникнуть затруднения с репликацией сообщений из всех других мест и
  придется выяснять, о каком именно брокере идет речь.

### Показатели брокеров

#### Признаки текущего контроллера

Показатель указывает, является ли данный брокер текущим контроллером кластера. Проблема или два брокера являются
контроллерами в кластере, либо ни один не претендует им быть.

#### Размер очереди контроллера

Следует ожидать скачков показателя, но если это значение постоянно увеличивается или остается стабильно высоким и не
падает, то контроллер может зависнуть. Чтобы исправить ситуацию, необходимо переместить контроллер в другой брокер, что
требует выключения брокера, в котором в данный момент находится контроллер.

#### Коэффициент простоя обработчиков запросов

Kafka использует два пула потоков выполнения для обработки всех запросов клиентов: сетевые потоки и потоки обработчиков
запросов (также называемые потоками ввода/вывода). Первый не сильно потребляет ресурсы. Второй отвечает за обслуживание
самих запросов клиентов, к которому относятся чтение сообщений с диска или их запись на диск. Показатель «коэффициент
простоя обработчиков запросов» отражает долю времени (в процентах), в течение которого обработчики запросов не
используются. Чем меньше это значение, тем сильнее загружен брокер. Коэффициент простоя меньше 20 % указывает на
потенциальную проблему, а меньше 10 % — на возникшую проблему с производительностью.

#### Суммарная входящая скорость передачи данных

Выраженная в байтах в секунду, может оказаться полезной в качестве показателя количества сообщений, получаемых брокерами
от клиентов-производителей. Этот показатель удобен при определении того, когда нужно расширять кластер или выполнять
другие работы, связанные с масштабированием.

#### Суммарная исходящая скорость передачи данных

В данном случае он отражает исходящую скорость чтения потребителями данных. Исходящая скорость передачи может
масштабироваться не так, как входящая, благодаря способности Kafka с легкостью работать с несколькими потребителями.

#### Суммарная по всем топикам число входящих сообщений

Отражает количество отдельных сгенерированных в секунду входящих сообщений вне зависимости от их размера. Он полезен в
качестве дополнительного показателя трафика производителей.

#### Число разделов

Мониторинг этого показателя более интересен в случае кластера, в котором включено автоматическое создание топиков,
поскольку владелец кластера не всегда контролирует этот процесс.

#### Число ведущих реплик

Чрезвычайно важно регулярно проверять число ведущих реплик, возможно, даже настроить на его основе уведомление,
поскольку оно отражает несбалансированность кластера даже в случае идеального баланса реплик по числу и размеру.

#### Отключенные разделы

Он показывает число разделов кластера, у которых сейчас нет ведущей реплики. Раздел без ведущей реплики может возникнуть
по двум причинам:

* Останов всех брокеров, на которых находятся реплики данного раздела
* Ни одна согласованная реплика не может стать ведущей из-за расхождения числа сообщений в случае, когда отключен
  «нечистый» выбор ведущей реплики

#### Показатели запросов

Контролируют функционирование 50 типов запросов. Для каждого из них существует 8 показателей:

* Общее время — общее количество времени обработки запроса брокером от получения до отправки ответа.
* Время нахождения запроса в очереди — длительность времени, проведенного запросом в очереди после его получения, но до
  начала обработки.
* Локальное время — количество потраченного ведущей репликой на обработку запроса времени, включая отправку его на
  диск (но, возможно, не учитывая фактический сброс на диск).
* Удаленное время — время, которое пришлось потратить на ожидание ведомых реплик, прежде чем стало возможно завершить
  обработку запроса.
* Длительность притормаживания — промежуток времени, на который необходимо придержать ответ, чтобы замедлить
  запрашивающий клиент настолько, что будут удовлетворены настройки квот клиентов.
* Время нахождения ответа в очереди — количество времени, проводимого запросом в очереди перед отправкой запрашивающему
  клиенту.
* Длительность отправки запроса — количество времени, фактически затраченного на отправку запроса

У каждого показателя следующие атрибуты:
...
Показатели уровня топика очень схожи с описанными ранее показателями брокеров.

#### Показатели уровня раздела

Показатели уровня раздела обычно менее полезны в текущей работе, чем показатели уровня топика. Тем не менее в отдельных
ситуациях они могут оказаться полезными. В частности, показатели уровня раздела отражают объем данных (в байтах),
хранящийся в настоящий момент на диске для определенного раздела. Если их суммировать, можно узнать объем хранимых
данных отдельного топика, что удобно при выделении ресурсов для отдельных клиентов Kafka. Расхождение между размерами
двух разделов одного и того же топика может указывать на проблему с неравномерным распределением сообщений по
использовавшемуся при генерации ключу.

### Мониторинг JVM

#### Сборка мусора

Для JVM критически важно наблюдать за процессом сборки мусора (GC). Показатели это _Полные циклы GC_ и _Молодое
поколение GC_. У обоих важные атрибуты `CollectionCount` - число циклов CG, `CollectionTime` - продолжительность времени
сборки мусора с момента запуска JVM.

#### Мониторинг операционной системы из Java

JVM предоставят информацию об операционной системе с помощью `java.lang:type=OperatingSystem`. В ее составе есть два
полезных атрибута, данные по которым тут можно собрать, но непросто получить в операционной системе:
`MaxFileDescriptorCount` - максимальное допустимое для JVM число открытых дескрипторов файлов, и
`OpenFileDescriptorCount`. `MaxFileDescriptorCount` - соответствует числу открытых в данный момент дескрипторов.

### Мониторинг OC

Основное, что нужно отслеживать, — использование ресурсов CPU, оперативной памяти, дисков, дисковых операций
ввода/вывода и сети.

### Журналирование

Механизма журналирования, записывающих информацию в отдельные файлы на диске:

* `kafka.controller` на уровне `INFO`, он служит для получения информации конкретно о контроллере кластера. Его
  информация включает данные о создании и изменении топика, изменении состояния брокера, а также такие операции
  кластера, как выбор предпочтительной реплики и перемещения разделов.
* `kafka.server.ClientQuotaManager`, тоже уровня `INFO`. Он используется для отображения сообщений, связанных с квотами
  на
  операции генерации и потребления. Это полезная информация, но в главном файле журнала брокера она будет только
  отвлекать

Журналирование о состоянии потоков сжатия журналов: `kafka.log.LogCleaner`, `kafka.log.Cleaner` и
`kafka.log.LogCleanerManager` на уровне `DEBUG`. При обычном функционировании сведений не так уж много, так что можно
включить это журналирование по умолчанию, не опасаясь утонуть в информации.

## Мониторинг клиентов

### Показатель производителей

Все показатели производителя содержат идентификатор клиента-производителя в названии компонента.

#### Показатели производителя в целом

На какие атрибуты следует обратить внимание:

* `record-error-rate` - всегда должен быть равен 0, а если он больше 0, значит, производитель отменяет сообщения,
  которые пытается отправить брокерам Kafka.
* `request-latency-avg` - средняя длительность отправки запроса от производителя. Лучше определить эталонное значение
  этого параметра при нормальном функционировании и установить оповещение при его превышении
* `outgoing-byte-rate` - трафик сообщений в байтах в секунду
* `record-send-rate` - трафик в терминах числа сгенерированных сообщений в секунду
* `request-rate` - число отправленных производителями брокерам запросов в секунду
* `request-size-avg` - средний размер запросов, отправляемых брокерам производителями, в байтах
* `batch-size-avg` - средний размер отдельного пакета сообщений, состоящего по умолчанию из сообщений, предназначенных
  для отдельного раздела топика, в байтах
* `record-size-avg` - средний размер отдельной записи в байтах
* `records-per-request-avg` - среднее число сообщений в отдельном запросе от производителя
* `record-queue-time-avg` - среднее время (в миллисекундах), которое отдельному сообщению приходится ожидать в
  производителе после отправки его приложением и до фактической генерации его для Kafka

#### Показатели уровня брокера и топика

Все атрибуты этих компонентов аналогичны описанным ранее атрибутам компонентов общих показателей производителей, и смысл
их точно такой же, за исключением того, что они относятся к отдельному брокеру или топику.

### Показатель потребителей

#### Показатели диспетчера извлечения

В клиенте-потребителе компонент показателя по потребителю в целом приносит меньше пользы, поскольку интересующие нас
данные расположены не там, а в компонентах диспетчера извлечения.

Атрибуты показатели диспетчера извлечения по которым имеет смысл настроить мониторинг и оповещение:

* `fetch-latency-avg` - сколько времени занимает выполнение запросов к брокерам на извлечение
* `bytes-consumed-rate` - описывают потребляемый данным экземпляром клиента трафик сообщений в байтах в секунду
* `records-consumed-rate` - описывают потребляемый данным экземпляром клиента трафик сообщений в сообщениях
  в секунду
* `fetch-rate` - число выполняемых потребителем запросов на извлечение в секунду
* `fetch-size-avg` - средний размер этих запросов на извлечение в байтах
* `records-per-request-avg` - среднее число сообщений в каждом запросе на извлечение

#### Показатели уровня брокера и топика

Показатели, предоставляемые клиентом-потребителем по каждому из соединений брокера и каждому из потребляемых топиков,
как и в случае с клиентом-производителем, удобны для отладки проблем с потреблением, но отслеживать их регулярно,
вероятно, смысла нет.

#### Показатели координатора потребителя

Главная проблема, с которой сталкиваются потребители в результате выполнения действий по координации, — приостановка
потребления на время синхронизации группы потребителей. Для этого помогут следующие атрибуты:

* `sync-time-avg` - среднее время согласования в миллисекундах
* `sync-rate` - число операций согласования группы в секунду

Также полезные атрибуты:

* `commit-latency-avg` - показатель среднего времени, расходуемого на фиксацию смещения
* `assigned-partitions` - число разделов, назначенных для потребления клиенту-потребителю как отдельному участнику
  группы потребителей

### Квоты

Apache Kafka умеет притормаживать запросы клиентов ради предотвращения ситуации, когда один клиент перегружает весь
кластер. Эту возможность, выражаемую в терминах объема трафика, который клиент с конкретным идентификатором может
отправлять конкретному брокеру (в байтах в секунду), можно настроить как для клиентов-потребителей, так и для
клиентов-производителей. Их мониторинг — рекомендуемая практика, поскольку в какой-то момент в будущем они могут
оказаться включены и легче сразу начать их мониторинг, а не добавлять показатели потом.

## Мониторинг отставания

Предпочтительный метод мониторинга отставания потребителя — использование внешнего процесса, который может отслеживать
как состояние раздела в брокере (наблюдая за последним сгенерированным сообщением), так и состояние потребителя (
наблюдая за последним смещением, зафиксированным группой потребителей для данного раздела). Один из способов
использование `Burrow`.

## Сквозной мониторинг

Он позволяет взглянуть на состояние кластера Kafka с точки зрения клиента. Если вы отвечаете за работу только кластера
Kafka, а не клиентов, вам придется отслеживать и функционирование клиентов тоже. Для этого можно воспользоваться
утилитой Xinfra Monitor.

# Глава 14. Потоковая обработка

## Что такое потоковая обработка

Поток данных(data stream), поток событий(event stream), потоковые данные(streaming data) - абстрактное представление
неограниченного набора данных

Неограниченность(unbounded stream) - потенциально бесконечный размер и непрерывный рост

Поток данных следует использовать, при необходимости анализировать все бизнес-операции.

Характерные черты:

* Упорядоченность потоков событий. В отличие от таблицы в БД, где записи могут быть не упорядочены.
* Неизменность записей данных. Событие не отменяется, а добавляется новое событие с отменной старой.
* Повторяемость потоков событий.

Потоковая обработка - это парадигма программирования, основанная на непрерывной обработке без блокировки.

## Основные понятия потоковой обработки

### Топология

Приложение для обработки потоков включает одну или несколько топологий обработки. Топология обработки начинается с
одного или нескольких исходных потоков, которые проходят через граф потоковых процессоров(фильтрация, агрегация),
соединенных через потоки событий, пока результаты не будут записаны в один или несколько потоков-приемников.

### Время

Большинство потоковых приложений выполняют операции в соответствии с временными окнами. Виды времени в контексте системы
потоковой обработки:

* Время события. Момент времени, когда произошло отслеживаемое событие и создана запись. При обработке потоковых данных
  основное значение имеет именно время события.
* Время добавления информации в журнал. Время поступления события в брокер Kafka и сохранения его там, также называемое
  временем приема.
...
При необходимости выполнения операций с несколькими событиями: подсчета числа событий по типам, вычисления скользящих
средних, объединения двух потоков данных для обогащения потока информации и т. д. В данном случае необходимо отслеживать
дополнительную информацию, например, сколько событий каждого типа встретилось нам за час, хранить список всех требующих
объединения событий, сумм, средних значений и т. д. Это и называется состояние.

Типы состояния:

* Локальное (внутреннее) состояние. Состояние, доступное только конкретному экземпляру приложения потоковой обработки.
  Хранится и контролируется встроенной БД в оперативной памяти. Преимущество — быстрота работы с ним. Недостаток —
  возможности ограничены объемом доступной памяти.
* Внешнее состояние. Состояние, хранимое во внешнем хранилище данных, обычно в NoSQL. Преимущества — практически полное
  отсутствие ограничений размера и возможность доступа к нему из различных экземпляров приложения или даже
  различных приложений. Недостатки — увеличение времени задержки и привносимая еще одной системой дополнительная
  сложность, а также то, что приложение должно учитывать вероятность недоступности внешней системы

### Таблично-потоковый дуализм

Поток - последовательность событий, в которой каждое событие является причиной изменения данных.

Материализация (materializing) - при преобразовании потока данных в таблицу, необходимость применить все содержащиеся в
этом потоке изменения.

### Временные окна

Большинство операций над потоками данных — оконные, то есть оперирующие временными интервалами. Необходимо знать
следующее об временных окнах: размер окна, насколько часто окно сдвигается, в течение какого времени сохраняется
возможность обновления окна.

### Гарантии обработки

Kafka Streams использует транзакции Kafka для реализации гарантий «только один раз» для приложений потоковой обработки.

## Паттерны проектирования потоковой обработки

### Обработка события по отдельности

В этом паттерне приложение потоковой обработки читает события из потока, модифицирует каждое из них, после чего
генерирует события в другой поток.

### Обработка с использованием локального состояния

Большинство приложений потоковой обработки реализует агрегирование информации, особенно по окнам. Это требует сохранения
состояния. Необходимо учитывать несколько моментов:

* Использование памяти. Локальное состояние идеально помещается в доступной экземпляру приложения оперативной памяти.
* Сохраняемость. Необходимо гарантировать, что состояние не будет утрачено при останове экземпляров приложения и есть
  возможность восстановить его при повторном их запуске или замене на другой экземпляр.
* Переназначение. Иногда разделы переназначаются другому потребителю.

### Многоэтапная обработка/повторное разделение на разделы

Для глобальной агрегации (например, топ-10 событий) нам нужен двухэтапный подход. На первом этапе выполняется агрегация
по типу с локальным агрегированием состояния. Результаты записываются в новую топик с одним разделом, которая
используется другим приложением (по аналогии с map-reduce).

### Обработка с применением внешнего справочника: соединение потока данных с таблицей

Иногда для потоковой обработки необходима интеграция с внешним по отношению к потоку производителем данных. Но внешний
справочник существенно увеличивает время обработки каждой записи. Eсли можно захватывать все происходящие с таблицей
базы данных изменения в поток событий, то можно было бы организовать прослушивание этого потока заданием, которое
выполняет потоковую обработку, и обновлять кэш в соответствии с событиями изменения базы данных. Процесс захвата
вносимых в базу данных изменений в виде событий потока данных носит название «сбор данных об изменениях» (change data
capture, CDC). В Kafka Connect есть множество коннекторов, предназначенных для выполнения CDC и преобразования таблиц
базы данных в поток событий изменения.

### Соединение таблицы с таблицей

Соединение двух таблиц всегда выполняется без окна и позволяет увидеть текущее состояние обеих таблиц на момент
выполнения операции. Эквисоединение - обе таблицы имеют одинаковый ключ, который разделен одинаковым образом, и поэтому
операция соединения может быть эффективно распределена между большим количеством экземпляров приложений и машин. Kafka
Streams также поддерживает соединение двух таблиц по внешнему ключу — ключ одного потока или таблицы соединяется с
произвольным полем из другого потока или таблицы.

### Соединение потоков

Соединение двух потоков данных означает соединение полной истории событий и поиск соответствий событий из одного потока
событиям из другого, относящимся к тем же временным окнам с такими же ключами. Поэтому соединение потоков называют также
оконным соединением (windowed-join).

### Внеочередные события

Это означает, что такое приложение должно:

* распознать несвоевременное поступление события, для чего прочитать время события и определить, что оно меньше
  текущего;
...
* Новая версия приложения потоковой обработки, и нужно организовать обработку этой версией того же потока событий
* В приложение потоковой обработки есть программная ошибка.

### Интерактивные запросы

В большинстве случаев пользователи приложений потоковой обработки получают результаты обработки, считывая их из топика
вывода. В некоторых случаях, желательно использовать самый короткий путь и считать результаты из самого хранилища
состояния.

## Kafka Streams в примерах

В Apache Kafka есть два потоковых API — низкоуровневый Processor API и высокоуровневый Streams DSL. Создание приложения,
задействующего API DSL, всегда начинается с формирования с помощью StreamsBuilder топологии обработки — ориентированного
ациклического графа (DAG) преобразований, применяемых ко всем событиям потоков. Затем на основе топологии создается
исполняемый объект KafkaStreams.

### Подсчет количества слов

Паттерн обработки отдельных событий. Полный пример http://www.bit.ly/2ri00gj
...
Демонстрирует возможности выполнения операций оконного агрегирования над потоком данных. Полный
пример http://www.bit.ly/2r6BLm1
...
Паттерн соединения двух потоков на основе временного окна. Полный пример http://www.bit.ly/2sq096i.
...
Любое потоковое приложение реализует и выполняет одну топологию. Топология, называемая в других фреймворках потоковой
обработки также DAG(directed acyclic graph — ориентированный ациклический граф), представляет собой набор операций и
преобразований, через которые проходят все события на пути от входных данных до результатов.

### Оптимизация топологии

Выполнение приложения Kafka Streams представляет собой трехэтапный процесс:

1. Логическая топология определяется путем создания объектов KStream и KTable и выполнения над ними операций DSL, таких
   как filter и join.
...
На втором этапе, когда физическая топология создается на основе логических определений, можно применить общую
оптимизацию плана. В основном связана с повторным использованием топиков, где возможно.

### Тестирование топологии

Основным инструментом тестирования приложений Kafka Streams является TopologyTestDriver. Эти тесты выглядят как обычные
модульные тесты. Интеграционные тесты для Kafka Streams существует два популярных фреймворка: EmbeddedKafkaCluster и
Testcontainers.

### Масштабирование топологии

Kafka Streams масштабируется за счет того, что внутри одного экземпляра приложения могут работать несколько потоков
выполнения, а также благодаря балансировке нагрузки между распределенными экземплярами приложения. Движок Streams
распараллеливает выполнение топологии, разбивая ее на задачи. Число задач определяется движком Streams и зависит от
числа разделов в обрабатываемых приложением топиках. Каждая задача отвечает за какое-то подмножество разделов: она
подписывается на эти разделы и читает из них события. Для каждого прочитанного события задача выполняет по порядку все
подходящие для этого раздела шаги обработки, после чего записывает результаты в приемник. Эти задачи — базовая единица
параллелизма в Kafka Streams, поскольку любую задачу можно выполнять независимо от остальных.

### Как пережить отказ

Kafka Streams использует предоставляемую Kafka координацию потребителей с целью обеспечения высокой доступности для
задач. Если задача завершилась неудачей, но есть другие активные потоки или экземпляры потокового приложения, ее можно
перезапустить в одном из доступных потоков.

## Сценарии использования потоковой обработки

_Лучше прочитать в книге_